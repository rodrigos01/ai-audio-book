...
export default function TitleDetail() {
  ...
  const loadChapters = async () => {
    try {
      const data = await api.getChapters(id);
      setChapters(data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleAddChapter = async (e) => {
    ...
    setCreating(true);
    try {
      await api.createChapter(id, newContent, selectedVoice, chapterName);
      setNewContent('');
      setChapterName('');
      loadChapters();
    } catch (e) {
  ...
  const handleRenameChapter = async (chapterId) => {
    ...
    try {
      await api.updateChapter(chapterId, editChapterName);
  ...
  return (
    <div className="flex-col gap-8 pb-10">
      ...
      <section className="animate-fade-in">
        <h2 style={{ fontSize: '1.25rem', color: 'var(--md-sys-color-on-surface)', marginBottom: '1.5rem', fontWeight: 400, display: 'flex', alignItems: 'center', gap: '0.75rem', paddingLeft: '0.5rem' }}>
          <md-icon><span className="material-symbols-outlined">format_list_numbered</span></md-icon>
          Chapters
        </h2>
