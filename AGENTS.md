...
1. **No auth.** Single-user/local-dev scope. `firebase.rules` (gitignored, not in this repo checkout) presumes Firebase Auth via an `owner_id` field, but this backend uses the Admin SDK exclusively, which bypasses Firestore security rules entirely — so those rules are currently dormant.
